...
class CDbCommandBuilder extends CComponent
{
	...
	//Francisco Seleccionar solo los activos
	private $alias_global='';
	...
	public function createFindCommand($table,$criteria,$alias='t')
	{
		...
		//Francisco: Seleccionar solo los activos
		if(isset(Yii::app()->modules['eal']['table']) && $table->rawName!='`'.Yii::app()->modules['eal']['table'].'`')
			$this->alias_global=$alias;
		else
			$this->alias_global='';
	...
	{
		$this->ensureTable($table);
		//Francisco: no elimnar sino cambiar estatus
		if(isset(Yii::app()->modules['eal']['table']) && isset(Yii::app()->modules['eal']['column'])
		 && isset(Yii::app()->modules['eal']['deleteValue']) && $table->rawName!='`'.Yii::app()->modules['eal']['table'].'`')
			$sql="UPDATE {$table->rawName} SET ". Yii::app()->modules['eal']['column'] ."=" . Yii::app()->modules['eal']['deleteValue'];
		else
			//aqui acaba
	...
	{
		//Francisco modificacion para cosas inactivas
		if($this->alias_global!=='')
			if($condition!='')
				return $sql.' WHERE '. $this->alias_global . '.estatus_did != '. Yii::app()->modules['eal']['deleteValue'] .' AND '.$condition;
			else
				return $sql.' WHERE '. $this->alias_global . '.estatus_did != '. Yii::app()->modules['eal']['deleteValue'] ;
		else if($condition!='')
