...
class ModelCode extends CCodeModel
{
	...
	public function generateLabels($table)
	{
		...
		foreach($table->columns as $column)
		{
			...
			if(strcasecmp(substr($label,-3),' dt')===0)
				$label=substr($label,0,-3);
	...
	protected function generateRelationName($tableName, $fkName, $multiple)
	{
		if(strcasecmp(substr($fkName,-3),'did')===0 &&  strcasecmp($fkName,'did'))
			...
			$relationName=rtrim(substr($fkName, 0, -3),'_');
		else if(strcasecmp(substr($fkName,-2),'id')===0 && strcasecmp($fkName,'id'))
			$relationName=rtrim(substr($fkName, 0, -2),'_');
		else if(strcasecmp(substr($fkName,-2),'dt')===0 && strcasecmp($fkName,'dt'))
				$relationName=rtrim(substr($fkName, 0, -2),'_');
		else
			$relationName=$fkName;
